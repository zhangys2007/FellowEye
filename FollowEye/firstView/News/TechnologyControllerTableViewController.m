...
#define kNEWSURL @"http://c.m.163.com/nc/article/headline/T1348649580692/%d-20.html"
...
@interface TechnologyControllerTableViewController ()<NetworkEngineDelegate>
...
@implementation TechnologyControllerTableViewController
-(void)dealloc
{
...
- (void)networkDidFinishLoading:(NetworkEngine *)engine withInfo:(id)info{
    ...
    NSArray *carousItem = dict[@"T1348649580692"];
...
- (void)play{
    ...
    if (self.carsource.count != 0)
    {
        ...
        UITapGestureRecognizer *tap = [[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(getTap:)]autorelease];
...
-(void)getTap:(UITapGestureRecognizer *)sender
{
    int i = 0;
    ...
    {
        [self.delegate PassValue:sender withInfo:model withMark:i];
    }
}
